...
class EmptyRanking:
    ...
    def evaluate(self, references, source="European Chemicals Agency (ECHA)"):
        compound_keys = list(references.keys())

        scores = {}

        for compound in compound_keys:
            scores[compound] = self._rank(references[compound], source)

        return scores

    def _rank(self, reference, source):
        ...
        for key in ref_keys:
            if reference[key].source_name == source:
