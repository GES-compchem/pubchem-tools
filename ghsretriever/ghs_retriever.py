...
class CompoundLibrary:
    def __init__(self, compound_list, style="CAS"):
        ...
        self._timings = queue.Queue() # used to calculate the number of requests to PUBCHEM
    ...
    def _populate_references(self, compound_list, style):
        ...
        while compounds_queue.empty() is False:
            ...
            for thread_num in range(10): # create 10 workers per second
                ...
                if (compounds_queue.empty() is False) and (within_pubchem_limits):
                    ...
                    worker = threading.Thread(target=self._compound_to_ghs,
                                              args=[compound, style, session])
                    new_threads.append(worker)
                    worker.start()

            print('\nProgress: ', str(self.results_queue.qsize())+"/"+str(self.compound_num)+"\n")

            within_pubchem_limits = self._check_timings() # limits speed based on running time
        ...
        print("Total elapsed time: ", datetime.timedelta(seconds = time.time()-self._start_time))
        ...
        for compound in compound_keys:
            ...
            for ref in references_keys:
                ...
                if (companies is None) or (companies <= max_count):
                    ...
                    if last_ref is None:
                        print('Ref ', ref, ' is the first entry with a populated companies field. Setting last_ref and max count.')
                        last_ref = ref
                        max_count = companies
                    elif last_ref is not None:
                        print('Deleting reference ', last_ref, " because ref ", ref, "has a higher count of companies")
